...
void drawLandmarks(Mat& im, vector<Point2f>& landmarks)
{
    ...
    //change code to be !=68 when it should be == 68 - so it will draw the lines connecting dots
    ...
    { // If the number of points is not 68, we do not know which 
      ...
      // one dot per landamrk. 
        ...
        for (int i = 0; i < landmarks.size(); i++)
        {
                ...
                //color ordering as BGR
                //circle(im, landmarks[i], 3, Scalar(15, 15, 255), FILLED);
          
        }

        //adding this to see what each iteration does
        //circle(im, landmarks[value], 3, COLOR, FILLED);
        //cout << "landmarks[" << value << "] x is: " << landmarks[value].x << "   y is:  " << landmarks[value].y << endl;
    }
